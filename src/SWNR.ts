export class SWNR implements SystemApi {
    ...
    actorSheetAddTab(sheet, html, actor, tabData: { id: string, label: string, html: string }, tabBody: string): void {
        ...
        const tabItem = $('<a class="sheet-body" data-group="primary" data-actionn="tab" data-tab="' + tabData.id + '" title="' + tabData.label + '">' + tabData.label + '</a>');
        tabs.append(tabItem);
        const body = $(html).find(".window-content");
        $(html).find(".beavers-tab-content").remove();
        const tabContent = $('<section class="beavers-tab-content tab flexcol sheet-body" data-group="primary" data-tab="' + tabData.id + '"></section>');
